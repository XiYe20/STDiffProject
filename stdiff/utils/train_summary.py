...
class VisCallbackVAE(Callback):
    @pl.utilities.rank_zero_only
    def on_train_epoch_end(self, trainer, pl_module):
        if trainer.current_epoch % pl_module.cfg.Training.log_freq == 0:
            pl_module = pl_module.eval()

            dataloader = trainer.datamodule.train_dataloader()
            save_dir = Path(pl_module.cfg.Env.ckpt_save_dir).joinpath(f'train_gifs_epoch{trainer.current_epoch}')
            self.vis_samples(dataloader, pl_module, save_dir)

    def vis_samples(self, dataloader, pl_module, save_dir):
        Vo, Vp, _, _ = next(iter(dataloader))
        Vo, Vp = Vo.to(pl_module.device), Vp.to(pl_module.device)
        x = torch.cat([Vo, Vp], dim =1)
        N, T, _, _, _ = x.shape

        with torch.no_grad():
            posterior = pl_module.encode(x.flatten(0, 1))
            rec = pl_module.decode(posterior.sample())
        
        rec = rearrange(rec, '(N T) C H W -> N T C H W', N=N, T=T)
        idx = min(N, 4)

        visualize_batch_clips(x[0:idx, :, ...], rec[0:idx, :, ...], rec[0:idx, :, ...], save_dir, renorm_transform = lambda x: (x+1)/2., desc = 'vae')
